...
<div class="container py-4">
    ...
    <form method="POST" action="{{ route('admin.jobs.update', ['job' => $job->id]) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="en_title" class="form-control" value="{{ old('en_title', $job->en_title) }}" required>
            @error('en_title') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <div class="mb-3">
            <label>{{ __('lang.ar_title') }}</label>
            <input type="text" name="ar_title" class="form-control" value="{{ old('ar_title', $job->ar_title) }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="job_type" class="form-control" value="{{ old('job_type', $job->job_type) }}">
        ...
        <div class="mb-3">
            ...
            <textarea name="en_description" id="en_description" class="form-control">{{ old('en_description', $job->en_description) }}</textarea>
            @error('en_description') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <div class="mb-3">
            <label>{{ __('lang.ar_description') }}</label>
            <textarea name="ar_description" id="ar_description" class="form-control">{{ old('ar_description', $job->ar_description) }}</textarea>
