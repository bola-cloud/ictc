        ...
        <div class="text-center mb-5">
            <div class="section-header d-flex justify-content-center align-items-center mb-4">
                <h2 class="section-title" style="font-family: 'Georgia', serif; font-weight: bold; font-size: 2.8rem; color: #333;">
                    <span style="border-bottom: 4px solid #3e4756; padding-bottom: 5px;">{{ __('lang.job_vacancies') }}</span>
                </h2>
            </div>
        </div>
        @forelse($vacancies as $vacancy)
            <div class="mb-5 p-5" style="background-color: #404b62; border-radius: 60px;">
                <div style="background-color: #f8ecd4; padding: 40px; border-radius: 10px; font-size: 1rem; color: #333; font-family: Georgia, serif;">
                    <p><strong class="job-title">{{ __('lang.job_title') }}:</strong>
                        {{ app()->getLocale() === 'ar' ? $vacancy->ar_title : $vacancy->en_title }}
                    </p>

                    <p><strong class="job-title">{{ __('lang.location') }}:</strong>
                        {{ $vacancy->location ?? __('lang.not_specified') }}
                    </p>

                    <p><strong class="job-title">{{ __('lang.job_type') }}:</strong>
                        {{ $vacancy->job_type ?? __('lang.not_specified') }}
                    </p>

                    <p><strong class="job-title">{{ __('lang.application_deadline') }}:</strong>
                        {{ $vacancy->deadline ? \Carbon\Carbon::parse($vacancy->deadline)->format('F d, Y') : __('lang.not_specified') }}
                    </p>

                    <p><strong class="job-title">{{ __('lang.job_description') }}:</strong></p>
